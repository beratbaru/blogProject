...
<div class="container mx-auto p-4">
    <div class="sticky top-0 bg-gray-100 z-10 py-3 border-b">
        <div class="flex justify-between items-center">
            <h4 class="text-lg font-semibold">Ürün Listesi</h4>
            <div class="flex items-center">
                <div class="md:hidden relative">
                    ...
                    <div class="absolute top-full right-0 bg-white shadow-md mt-2 rounded-md hidden" id="menuDropdown">
                        ...
                        <a href="{{ url('product/create') }}" class="block px-6 py-2 hover:bg-gray-200 rounded-md">
                ...
                <div class="hidden md:flex gap-2">
                    ...
                    <a href="{{ url('product/create') }}" class="text-gray-600 text-xl hover:text-gray-800">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-4 mt-12">
        ...
        <div class="col-span-full text-center py-4">Ürün bulunamadı.</div>
        @else
        @foreach ($products as $product)
        <div class="bg-white shadow-lg rounded-md p-4 flex flex-col justify-between relative">
            <div>
                <h5 class="text-lg font-bold mb-2">{{ $product['product_name'] }}</h5>
                <p class="text-gray-600 mb-4">{{ $product['description'] }}</p>
            </div>
            <div class="mt-auto">
                <p class="text-gray-800 font-semibold mb-4">{{ $product['product_price'] }}₺</p>
                <div class="absolute right-4 top-1/2 transform -translate-y-1/2 flex flex-col gap-4">
                    <!-- Edit Button (Icon) -->
                    <a href="{{ route('product.edit', $product['id']) }}" class="text-xl">
                        <i class="fas fa-edit"></i>
                    </a>
                    <!-- Show Button (Icon) -->
                    <a href="{{ route('product.show', $product['id']) }}" class="text-xl">
                        <i class="fas fa-eye"></i>
                    </a>
                    <!-- Delete Button (Icon) -->
                    <form action="{{ route('product.destroy', $product['id']) }}" method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?');">
                        @csrf
                        @method('DELETE')
